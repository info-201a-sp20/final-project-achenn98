...
abortion_rate_and_clinics <- joined_df %>%
  mutate(
    new_num_abortion_clinics = as.numeric(joined_df$Num_of_abortion_clinics_2017),
    new_abortion_rate = as.numeric(joined_df$Abortion.rate..the.no..of.abortions.per.1.000.women.aged.15.44...by.state.of.occurrence..2017)
  ) %>%
  group_by(Region) %>%
  summarize(
...
scatterplot <- ggplot(data = abortion_rate_and_clinics) +
  geom_point(mapping = aes(x = total_abortion_clinics, y = total_abortion_rate, color = Region)) +
  labs(
    title = "Total Abortion Rates x Total Abortion Clinics by Region",
